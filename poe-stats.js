...
(async () => {
    ...
    console.log("🚀 Starting PoE Diagnostic Sync...");
    const poeReport = [];
    ...
    for (const venueData of swList) {
        ...
        try {
            ...
            for (const sw of venueData.switches) {
                ...
                // PoE stats
                console.log("📊 Crawling Port Info...")
                ...
                if (!isNaN(iSitePort)) {
                    console.log(`Specified port: ${iSitePort}.`);
                ...
                async function togglePoE(page, targets, targetState) {
                    ...
                    console.log(`🔘 Setting PoE to ${targetState ? 'ON' : 'OFF'} for ports: ${targets.join(', ')}`);
                    ...
                    // hit save
                    await saveBtn.click();
                    console.log("💾 Save clicked. Waiting for VLAN confirmation modal...");
                    ...
                    try {
                        ...
                        console.log("✅ VLAN Confirmation 'Yes' clicked.");
                ...
                if (targetsToReset.length > 0) {
                    try {
                        console.log(`🔌 PHASE 1: Deep Reset (Enable PoE Toggle)`);
                        
                        // 1. Enter the first port's summary
                        ...
                        console.log("✅ Initial Batch Warning dismissed.");

                        // 4. Select ports in the grid
                        for (const portNum of targetsToReset) {
                            await page.locator(`#port_${portNum}`).click();
                        }

                        // 5. Expand the PoE Accordion
                        await page.click('#hNsaAccordHeadSettng');

                        // 6. TOGGLE OFF SEQUENCE
                        await togglePoE(page, targetsToReset, false);
                        
                        console.log("⏱️ Waiting 30s for full capacitor drain...");
                        await page.waitForTimeout(30000);

                        // 7. TOGGLE ON SEQUENCE
                        await togglePoE(page, targetsToReset, true);
                        console.log("🎉 Deep Reset complete.");

                        // --- PHASE 2: OPTIONAL POWER CYCLE FALLBACK ---
                        // After toggle, wait 10s to see if link comes up. If not, Power Cycle.
                ...
                while (attempts < 2 && !healthy) {
                    console.log(`⚡ Phase 2: Power Cycle Attempt ${attempts + 1}/2...`);
                    try {
                        await page.click('a:has-text("PoE Management")');
                        await page.waitForURL('**/devices/switch/PoE');

                        for (const portNum of targetsToReset) {
                            await page.click(`.ethernet-count:has-text("${portNum}")`);
                        }
                        await page.click('#btnSavePowerCyclePrts');
                        await page.waitForTimeout(10000); // Wait for cycle to trigger

                        // Short verification crawl here? 
                        // If traffic > 0, set healthy = true
                        attempts++;
                    } catch (e) { break; }
                }
                if (!healthy && attempts >= 2) {
                    console.log(`🚫 Failed after 3 total attempts (1 PoE Toggle, 2 Power Cycles). Skipping.`);
                }
        ...
        console.log("\n✨ Diagnostic Complete! Check 'poe-stats-report.json'.");
    }
})();